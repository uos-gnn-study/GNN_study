...
class GATLayer(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.linear_weight = nn.Parameter(
            torch.randn(num_heads, in_channels, out_channels)
        )
    ...
    def forward(self, x: Tensor, edge_index: Tensor) -> Tensor:
        linear_transformed_input = torch.matmul(
            self.linear_weight, torch.transpose(torch.unsqueeze(x, dim=0), 1, 2)
        )
        ...
        attention_coefficient = self.leakyrelu(
            torch.matmul(
                query_tensor, torch.transpose(key_tensor, 1, 2)
            )  # self.leakyrelu = nn.LeakyReLU(negative_slope=0.2) <- 논문에서 0.2로 함
        )

        adjacency_matrix = to_dense_adj(
            edge_index=edge_index, max_num_nodes=self.num_nodes
        )
        ...
        output = torch.reshape(
            torch.matmul(masked_attention_coefficient, value_tensor).permute(1, 0, 2),
...
class GATLastLayer(GATLayer):
    ...
    def forward(self, x: Tensor, edge_index: Tensor) -> Tensor:
        ...
        output = torch.mean(
            torch.matmul(masked_attention_coefficient, value_tensor), dim=0
        )
